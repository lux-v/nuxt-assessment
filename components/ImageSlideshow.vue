<template>
	<div class="slideshow-container">
		<div
			...
			@mouseenter="startSlideshow(page.featuredImage)"
...
const startSlideshow = (featuredImage) => {
	...
	const featuredImageIndex = slideshowImages.value.findIndex(
...
const addFeaturedImagesInSlideshow = () => {
	...
	const imagesNotInSlideshow = pages
		.filter(
			(page) =>
				!slideshowImages.value.some(
					(slide) => page.featuredImage.id === slide.id
				)
		)
...
const classes = (index) => {
	return ['slideshow-image', { active: index === currentIndex.value }];
};
